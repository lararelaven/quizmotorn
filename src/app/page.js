...
export default function Home() {
  ...
  useEffect(() => {
      ...
      if (user) {
        dispatch({
          ...
          payload: {
            email: user.email,
            id: user.id,
            name: 'Lärare' // Du kan hämta riktigt namn från metadata om du vill
          }
        });
      }
    };
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_IN' && session) {
        ...
        // Här kan du också behöva en 'LOGOUT' action i din reducer för att rensa state
      }
    });
  ...
  const renderToolbar = () => (
    <div className="fixed bottom-4 right-4 bg-black/50 backdrop-blur text-white p-2 rounded-lg shadow-2xl flex gap-2 z-50 text-[10px] border border-white/10 opacity-50 hover:opacity-100 transition-opacity">
      <button onClick={() => dispatch({ type: 'RESET_APP' })} className="px-2 py-1 bg-gray-700 rounded hover:bg-gray-600">Reset</button>
    </div>
  );
